...
class DPmixGGM
{
 ...
 public:
  ...
  //--------- Functions --------------
 public:
	DPmixGGM (Real *data, myInt L_start, myInt n_obs, myInt p_model, Real edgeInclusionProb, ifstream& initfile);
	DPmixGGM (State a);
	~DPmixGGM ();
...
DPmixGGM::DPmixGGM (State a)
{
  ...
  n = a->n; p = a->p; L = a->L; X = a->X; alpha = a->alpha; plp = a->plp;
  xi = new myInt[n]; for(i=0; i<n; i++) { xi[i] = a->xi[i]; }
  graphlist = new LPGraph[L]; pll = new Real[L];
  for(i=0; i<L; i++) { graphlist[i] = new Graph(); graphlist[i]->InitGraph(p); graphlist[i]->CopyGraph(a->graphlist[i]); pll[i] = a->pll[i]; }
...
DPmixGGM::~DPmixGGM()
{
  myInt i; delete[] xi; for(i=0; i<L; i++) { delete graphlist[i]; }; delete[] graphlist; delete[] pll;
}


void DPmixGGM::RandomStartAllXi (myInt L_start)
{ 
  for(myInt i=0; i<n; i++) { xi[i] = (myInt) (L_start*gsl_ran_flat(rnd, 0.0, 1.0))/1; }
}


void DPmixGGM::RandomStartAllG (myInt L_start, Real edgeInclusionProb)
{
  myInt i,k,l; bool temp;
  ...
  {   for(k=0; k<p-1; k++)
	...
	{ temp = (gsl_ran_flat(rnd, 0.0, 1.0) < edgeInclusionProb);
	  graphlist[i]->Edge[k][l] = temp; graphlist[i]->Edge[l][k] = temp;	  
	}
      }
      graphlist[i]->Edge[p-1][p-1] = 0;
      TurnFillInGraph(graphlist[i]); graphlist[i]->GenerateAllCliques();
  }
...
{
  myInt i; for(i=0; i<L; i++) { delete graphlist[i]; }; delete graphlist; delete pll;  
  L = L_start; graphlist = new LPGraph[L]; for(i=0; i<L; i++) { graphlist[i] = new Graph(); graphlist[i]->InitGraph(p); }; pll = new Real[L];
...
void DPmixGGM::ReadState (ifstream& initfile)
{
  ...
  int supern, superp; initfile >> supern; initfile >> superp; initfile >> L; //cout << "supern = " << supern << " superp = " << superp << " L = " << L << endl; fflush(stdout);
  for(i=0; i<n; i++) { initfile >> xi[i]; xi[i]--; }; for(i=n; i<supern; i++) { initfile >> j; }
  graphlist = new LPGraph[L]; pll = new Real[L];
  for(i=0; i<L; i++)
  {   graphlist[i] = new Graph(); graphlist[i]->InitGraph(p);
      for(k=0; k<p; k++) { for(l=0; l<p; l++) { initfile >> graphlist[i]->Edge[k][l]; }; for(l=p; l<superp; l++) { initfile >> j; } }
      for(k=p; k<superp; k++) { for(l=0; l<superp; l++) { initfile >> j; } }
      TurnFillInGraph(graphlist[i]); graphlist[i]->GenerateAllCliques();
  }  
...
{
  myInt i,j,l,q,r; Real score = plp; for(l=0; l<L; l++) { score += pll[l]; }
  
  out << L << " " << score << " " << itr << " ";
  for(i=0; i<n; i++) { out << xi[i] << " "; }
  for(l=0; l<L; l++) { for(q=0; q<p-1; q++) { for(r=q+1; r<p; r++) { out << graphlist[l]->Edge[q][r] << " "; } } }  
  for(l=0; l<L; l++)
...
void DPmixGGM::CopyState (State a)
{
  ...
  n = a->n; p = a->p; X = a->X; alpha = a->alpha; plp = a->plp;
  for(i=0; i<n; i++) { xi[i] = a->xi[i]; }; 
  if(L!=a->L)
  {	L = a->L; pll = new Real[L]; for(i=0; i<oldL; i++) { delete graphlist[i]; }; delete[] graphlist;
	graphlist = new LPGraph[L]; for(i=0; i<L; i++) { graphlist[i] = new Graph(); graphlist[i]->InitGraph(p); }
  }
  for(i=0; i<L; i++) { graphlist[i]->CopyGraph(a->graphlist[i]); pll[i] = a->pll[i]; }
  
}


#ifdef JEFFREYS_PRIOR
struct f_params { int n; int m; int k; };
double f (double beta, void *params)
{	struct f_params *iparams = (f_params *) params;
	int k = iparams->k; int n = iparams->n; int m = iparams->m;
	double s; double sum = 0; for(int j=1; j<m; j++) { s = beta+j; sum += ((Real) j)/(s*s); }
	return exp(lgamma(beta) - lgamma(beta+n) + lgamma(n+1.0) + (k-0.5)*log(beta) + log(sqrt(sum)));
}

Real DPmixGGM::lA (myInt the_n, myInt m, myInt k)
{
  struct f_params params = { the_n, m, k }; F.function = &f; F.params = &params;
  double result, error; gsl_integration_qagiu (&F, 0.0, 1e-7, 1e-7, GSL_INTEGRATION_GRIDSIZE, w, &result, &error);
  return log(result);
...
//--------------------------------------------------------------------
// This returns the partition prior probability -- normalising constant ignored assuming alpha is fixed
// maxL is maximum number of clusters, effective number of clusters can be smaller
Real DPmixGGM::partitionlogPrior(myInt maxL, myInt* thexi, Real thealpha)
{
  myInt i, j; Real siz; Real pri = 0; myInt effectiveL = 0;
  for(i=0; i<maxL; i++) { siz = 0.0; for(j=0; j<n; j++) { if(thexi[j]==i) siz = siz+1; }; if(siz>0) { pri += lgamma(siz); effectiveL++; } }
  
#ifdef JEFFREYS_PRIOR
  struct f_params params = { n, n, effectiveL }; F.function = &f; F.params = &params;
  double result, error; gsl_integration_qagiu(&F, 0.0, 1e-7, 1e-7, GSL_INTEGRATION_GRIDSIZE, w, &result, &error);
...
Real DPmixGGM::cluster_k_loglikelihood(myInt k, myInt *thexi, LPGraph thegraph)
{ 
  //-------- Variables, follows paper ---------------
  int i,j,ii; Real n0 = N0;
  Real *mu0 = new Real[p]; for(i=0; i<p; i++) mu0[i] = 0;
  Real *xbar = new Real[p]; Real *mu_bar = new Real[p];
  Real *D_prior = new Real[p*(p+1)/2]; Real *D_post = new Real[p*(p+1)/2];
  myInt n_sub = 0; Real J_G; Real Norm_terms;
  
  for(i=0; i<n; i++) { if(thexi[i]==k) n_sub++; } //count the number in cluster k
  if(n_sub==0) { delete[] D_post; delete[] D_prior; delete[] xbar; delete[] mu_bar; delete[] mu0; return(0); }
  ...
  for(i=0; i<p; i++)
  {	ii = i*(i+1)/2; 
	for(j=0; j<=i; j++) { D_post[ii+j] += (-(n_sub + n0) * mu_bar[i] *mu_bar[j] + n_sub * xbar[i] * xbar[j] + n0 * mu0[i] * mu0[j]); }
  }

  //------------ Calculate the score --------------------------------------------- 
  J_G = j_g_decomposable(thegraph, D_prior, D_post, DELTA0, n_sub, 0);
  Norm_terms = - (Real(n_sub*p)/2) * log_2_pi + Real(p)/2 * log(n0 / (n_sub + n0));

  delete[] D_post; delete[] D_prior; delete[] xbar; delete[] mu_bar; delete[] mu0;
